...
class MSDNet_overlap_imagenet(nn.Module):
    def __init__(self, args):
        super(MSDNet_overlap_imagenet, self).__init__()
        ...
        if self.nBlocks ==5:
            if self.args.method == 'overlap':
                ...
                if self.args.include_final == 'False':
                    ...
                    res.append(self.classifier[2](x3_aux))
                    x3_aux_1 = self.blocks[2]([x_com.clone().detach() for x_com in x2_aux])

                    x4_aux_1 = self.blocks[3](x3_aux_1)
                    res.append(self.classifier[3](x4_aux_1))
                    x4_aux_2 = self.blocks[3]([x_com.clone().detach() for x_com in x3_aux])

                    x5_aux_2 = self.blocks[4](x4_aux_2)
                    res.append(self.classifier[4](x5_aux_2))
                    ...
                    x1 = self.blocks[0](x)
                    res.append(self.classifier[0](x1))
                    x1_aux_2 = self.blocks[0](x.clone().detach())

                    x2 = self.blocks[1](x1)
                    res.append(self.classifier[1](x2))
                    x2_aux = self.blocks[1]([x_com.clone().detach() for x_com in x1])
                    x2_aux_2 = self.blocks[1](x1_aux_2)

                    x3_aux = self.blocks[2](x2_aux)
                    res.append(self.classifier[2](x3_aux))
                    x3_aux_1 = self.blocks[2]([x_com.clone().detach() for x_com in x2_aux])
                    x3_aux_2 = self.blocks[2](x2_aux_2)

                    x4_aux_1 = self.blocks[3](x3_aux_1)
                    res.append(self.classifier[3](x4_aux_1))
                    x4_aux_2 = self.blocks[3](x3_aux_2)
